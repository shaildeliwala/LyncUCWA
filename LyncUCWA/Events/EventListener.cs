...
namespace LyncUCWA.Events
{
    public static class EventListener
    {
        ...
        private static Lazy<ResponseForm> responseForm = new Lazy<ResponseForm>(() => new ResponseForm());
        ...
        {
            var respForm = responseForm.Value;
            while (!nextLink.IsEmpty())
            {
                ...
                if (httpResponse.IsSuccessStatusCode)
                {
                    ...
                    respForm.txtResponseContent.AppendText(String.Concat(jsonResponse, Environment.NewLine, Environment.NewLine));
                    if (!respForm.Visible)
                        respForm.Show();
        ...
        {
            var data = HttpUtility.UrlDecode(text, Encoding.UTF8);
            return data;
        }
